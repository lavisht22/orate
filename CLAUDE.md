...
- **Current screens**: Home (`.home` → `HomeView`), Instructions (`.instructions` → `CustomInstructionsView`)
...
The system prompt lives in `TranscriptionService.systemPrompt` (private static let). Custom instructions from UserDefaults are appended at the end via `buildSystemInstruction()`. The prompt is sent via Gemini's `system_instruction` API field, separate from the audio content.
